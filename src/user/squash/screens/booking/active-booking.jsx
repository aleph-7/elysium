...
function ActiveBooking() {
  ...
  useEffect(() => {
    ...
    const slotsData = await fetchAvailableSlots(current_date, "squash", 4);
    ...
    async function fetchCourts() {
      ...
      if (selectedTime) {
        const response = await fetch(`${SERVER_ROOT_PATH}/getAvailableCourts`, {
          ...
          body: JSON.stringify({ selectedTime: selectedTime,date:current_date,sport:"squash" }),
        });
  ...
  return (
    <form
      className="active-booking-form-squash"
      onSubmit={(e) => {
        handleSubmit(e);
        e.target.reset();
      }}
